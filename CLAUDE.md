...
Requires Node.js >=18 and Python 3 with `pyyaml`:
...
- **Claude code review** (`claude-code-review.yml`): Runs Claude Code Review on PRs via `anthropics/claude-code-action@v1`.
...
2. **Use YAML frontmatter** (`title`, `summary`, optional `order` for sort override)
